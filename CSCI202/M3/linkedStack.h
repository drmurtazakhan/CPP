...
void linkedStackType<Type>::copyStack(const linkedStackType<Type> &otherStack)
{
    ...
    else
    {
        ...
        {
            newNode = new nodeType<Type>;  // 1. Create a new node
            newNode->info = current->info; // 2. Copy the data
            newNode->link = nullptr;       // 3. Set the new node's link to null
            last->link = newNode;          // 4. Attach it to the previous node
            last = newNode;                // 5. Move 'last' forward
            current = current->link;       // 6. Move 'current' to the next original node
        }
    } // end else
} // end copyStack
