...
int const maxCard = 100;

//enum Bool { Bool=false, Bool=true };
...
{
	for (register int i = 0; i < set.card; ++i)
		if (elem == set.elems[i])
			return true;
	return false;
}
...
Bool operator == (Set &set1, Set &set2)
{
	if (set1.card != set2.card)
		return false;
	for (register int i = 0; i < set1.card; ++i)
...
Set operator + (Set &set1, Set &set2)
{
	...
	set1.Copy(res);
	for (register int i = 0; i < set2.card; ++i)
