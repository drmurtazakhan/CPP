...
void Stack<std::string>::printStack() {
        cout << "Special Strng Stack contents (top to bottom): ";
        for (int i = elements.size() - 1; i >= 0; --i) {
            transform(elements[i].begin(), elements[i].end(), elements[i].begin(), ::toupper);
            cout << elements[i] << " ";
        }
        cout << endl;
}
...
int main() {
    ...
    // Using Stack with different data types
